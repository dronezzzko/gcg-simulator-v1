...
| Decklist knowledge | Both AI players know both exact decklists (main and resource deck); hidden cards are resampled from the opponent's decklist minus the cards seen. | A benchmarking choice (the benchmark deck is a known meta list), not a game rule: the floor rules treat the main deck's contents as private and deck lists go only to the organizer. |
