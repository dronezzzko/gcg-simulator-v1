...
_PRONOUN = re.compile(r"^(?:it|them|they|those Units|those cards|that card|these cards|those)$", re.I)
...
def _friendly(sel: d.Sel) -> d.Sel:
    return sel if sel.side is not d.Side.ANY else d.Sel(d.Side.FRIENDLY, sel.loc, sel.filters, sel.top_n)
...
def _cond_opp_has_ex(m: re.Match[str], g: G) -> d.Cond:
    return d.Exists(d.Sel(d.Side.ENEMY, d.Loc.RESOURCE_AREA, (d.IsKind((d.CardKind.EX_RESOURCE,)),)))


def _cond_you_have_ex(m: re.Match[str], g: G) -> d.Cond:
    c: d.Cond = d.Exists(d.Sel(d.Side.FRIENDLY, d.Loc.RESOURCE_AREA, (d.IsKind((d.CardKind.EX_RESOURCE,)),)))
...
_CONDS: list[tuple[re.Pattern[str], Callable[[re.Match[str], G], d.Cond]]] = [
    ...
    (re.compile(r"^you (?:have )?use[d]? an EX Resource to play this (?:card|Unit)$", re.I), _cond_ex_used),
    (re.compile(r"^your opponent has an EX Resource(?: in play)?$", re.I), _cond_opp_has_ex),
    (re.compile(r"^you (don't |do not )?have an EX Resource(?: in play)?$", re.I), _cond_you_have_ex),
    ...
    (re.compile(r"^you have (\d+) (or more|or less|or fewer)? ?(.+?) in play$", re.I), _cond_count_you),
    (re.compile(r"^you have (.+?) in play$", re.I), _cond_have_in_play),
    (re.compile(r"^(\d+) (or more|or less|or fewer)? ?(.+?) (?:are|is) in play$", re.I), _cond_count_in_play),
    (re.compile(r"^there (?:is|are) (\d+) (or more|or less|or fewer)? ?(.+?) in play$", re.I), _cond_count_in_play),
    (
        re.compile(
    ...
    (
        re.compile(r"^you have (\d+) (or more|or less|or fewer)? ?(.+?) in (your trash|your hand)$", re.I),
        _cond_zone_count,
    ),
    (re.compile(r"^your opponent has (\d+) (or more|or less|or fewer)? ?(.+?) in (their trash)$", re.I), _cond_zone_count),
    (re.compile(r"^(you|your opponent) (?:are|is) Lv\.(\d+) (or higher|or lower)$", re.I), _cond_level),
    (
        re.compile(r"^(you|your opponent) (?:have|has) (\d+) (or more|or less|or fewer) cards in (?:your|their) hand$", re.I),
        _cond_hand,
    ),
    (re.compile(r"^(you|your opponent) (?:have|has) (\d+) (or more|or less|or fewer) Shields$", re.I), _cond_shields),
    (re.compile(r"^there are (\d+) (or more|or less|or fewer) enemy Shields$", re.I), _cond_enemy_shields),
    (re.compile(r"^(you|your opponent) (?:have|has) (\d+) (or more|or less|or fewer) cards in (?:your|their) deck$", re.I), _cond_deck),
    (
        re.compile(r"^(?:you are|this Unit is|it is) attacking (the enemy player|an enemy Unit|a damaged enemy Unit|an enemy Base)$", re.I),
        _cond_attacking,
    ),
    (re.compile(r"^you deploy(?:ed)? this (?:Unit|card) from your trash$", re.I), _cond_deployed_from_trash),
    ...
    (re.compile(r"^(this Unit|this|it|this Base|they) (?:has|have) " + KW_RE + r"$", re.I), _cond_self_has_kw),
    (re.compile(r"^(this Unit|it|this) has (\d+) ?(or more|or less)? (AP|HP)$", re.I), _cond_self_stat),
    (re.compile(r"^(this Unit|this|it|this Base|that Unit|the enemy Unit) is (.+)$", re.I), _cond_self_is),
...
_KWS = re.compile(r"<(Repair|Breach|Support|Blocker|First Strike|High-Maneuver|Suppression)(?:\s*(\d+))?>")
...
def continuous_from_predicate(pred: str, g: G) -> list[d.Continuous]:
    ...
    if m and _KWS.search(m.group(1)) and not _KWS.sub("", m.group(1)).replace("and", "").replace(",", "").replace("/", "").strip():
...
def _source_filters(desc: str) -> tuple[d.Side, tuple[d.Filter, ...]]:
    ...
    if not s:
        return d.Side.ANY, ()
    if s.lower() in ("an enemy", "enemies", "the enemy", "your opponent", "enemy effects", "an enemy effect"):
        return d.Side.ENEMY, ()
...
def _rule_predicate(p: str, g: G) -> d.RuleMod | None:
    ...
    if m:
        kind = {"battle ": d.DamageKind.BATTLE, "effect ": d.DamageKind.EFFECT}.get(m.group(2) or "", d.DamageKind.ANY)
        src = m.group(3) or ""
        ...
        return d.RuleMod(d.RuleKind.CANT_RECEIVE_DAMAGE, damage_kind=kind, source_filters=filters, source_side=side)
    ...
    if pl in ("can't have its ap reduced by enemy effects", "'s ap can't be reduced by enemy effects"):
...
def _tr_destroys_unit(m: re.Match[str], g: G) -> d.Trigger:
    ...
    return d.Trigger(d.Ev.DESTROYS_BY_BATTLE, self_only=False, subject=sel, target_filters=tf, include_self=inc)
...
def _tr_destroys_shield(m: re.Match[str], g: G) -> d.Trigger:
    ...
    sel, inc = _subject_sel(subj)
    return d.Trigger(d.Ev.DESTROYS_SHIELD_CARD, self_only=False, subject=sel, battle_only=battle, include_self=inc)
...
def _tr_deals_damage(m: re.Match[str], g: G) -> d.Trigger:
    ...
    sel, inc = _subject_sel(subj)
    return d.Trigger(d.Ev.DEALS_DAMAGE, self_only=False, subject=sel, battle_only=battle, target_filters=tf, include_self=inc)
...
def _tr_receives_damage(m: re.Match[str], g: G) -> d.Trigger:
    ...
    sel, inc = _subject_sel(subj)
    return d.Trigger(d.Ev.DAMAGED, self_only=False, subject=sel, by_enemy=by_enemy, battle_only=battle, include_self=inc)


def _tr_ex_placed(m: re.Match[str], g: G) -> d.Trigger:
    return d.Trigger(d.Ev.EX_RESOURCE_PLACED, self_only=False, subject=d.Sel(d.Side.FRIENDLY, d.Loc.RESOURCE_AREA))
...
def _tr_destroyed(m: re.Match[str], g: G) -> d.Trigger:
    ...
    sel, inc = _subject_sel(phrase)
    return d.Trigger(
        d.Ev.DESTROYED, self_only=False, subject=sel, include_self=inc or with_self, battle_only=battle, by_enemy=by_enemy
    )
...
def _tr_attacks(m: re.Match[str], g: G) -> d.Trigger:
    ...
    with_self, phrase = _with_self(subj)
    sel, inc = _subject_sel(phrase)
    return d.Trigger(d.Ev.ATTACKS, self_only=False, subject=sel, include_self=inc or with_self, target_filters=tf)
...
def _tr_paired_pilot(m: re.Match[str], g: G) -> d.Trigger:
    ...
    sel, inc = _subject_sel(phrase)
    return d.Trigger(d.Ev.PAIRED, self_only=False, subject=sel, pilot_filters=pf, include_self=inc or with_self)
...
def _tr_command(m: re.Match[str], g: G) -> d.Trigger:
    ...
    return d.Trigger(d.Ev.COMMAND_PLAYED, self_only=False, subject=d.Sel(d.Side.FRIENDLY, d.Loc.TRASH, filters))


_TRIGGERS: list[tuple[re.Pattern[str], Callable[[re.Match[str], G], d.Trigger]]] = [
    (
        re.compile(r"^when (this Unit|one of your .+?|a friendly .+?|another friendly .+?) destroys an enemy (.*?)Unit with (?:battle )?damage$", re.I),
        _tr_destroys_unit,
    ),
    (
        re.compile(r"^when (this Unit|one of your .+?|a friendly .+?) destroys an enemy (?:shield area card|Shield)( with (?:battle )?damage)?$", re.I),
        _tr_destroys_shield,
    ),
    (
        re.compile(r"^when (this Unit|one of your .+?) deals (battle )?damage to (an enemy .+?)$", re.I),
        _tr_deals_damage,
    ),
    (re.compile(r"^when (.+?) is deployed$", re.I), _tr_deployed),
    (
        re.compile(r"^when (this Unit|this Base|one of your .+?|a friendly .+?) receives ((?:enemy )?(?:battle |effect )?)damage( from an enemy)?$", re.I),
        _tr_receives_damage,
    ),
    (re.compile(r"^when you place an EX Resource$", re.I), _tr_ex_placed),
    (re.compile(r"^at the (start|end) of (your turn|your opponent's turn|the turn|each turn|a turn|every turn)$", re.I), _tr_turn),
    (re.compile(r"^when (.+?) is destroyed( with battle damage| with damage| by an? (?:enemy )?effect| by an enemy| by your opponent's effect)?$", re.I), _tr_destroyed),
    (re.compile(r"^when (this Unit|this Unit or .+?|another .+?|one of your .+?|an? .+?) attacks( an enemy Unit)?$", re.I), _tr_attacks),
    (re.compile(r"^when (this Unit|one of your .+?) is rested by an? (enemy |opponent's )?effect$", re.I), _tr_rested_by),
    (re.compile(r"^when this (?:rested )?Unit is set as active by an effect$", re.I), _tr_set_active),
    (re.compile(r"^when you pair an? ((?:\([^)]+\)(?:/\([^)]+\))*|blue|green|red|white|purple) )?Pilot with (this Unit|this Unit or one of your .+?|one of your .+?)$", re.I), _tr_paired_pilot),
    ...
    (re.compile(r"^when (one of your|your|an enemy) Shields? (?:is|are) destroyed$", re.I), _tr_shield_destroyed),
    (re.compile(r"^when (you|your opponent) discards?$", re.I), _tr_discard),
    (re.compile(r"^when (this Unit|one of your .+?) is returned to (?:its owner's|your) hand$", re.I), _tr_returned),
    (re.compile(r"^when you (?:play|activate) (?:an? )?(.+?)?Command card(?:'s 【Main】/【Action】)?$", re.I), _tr_command),
]
...
def _num(s: str | None, default: int = 1) -> int:
    if s is None:
        return default
    w = s.strip().lower()
    return {"a": 1, "an": 1, "one": 1, "two": 2, "three": 3}.get(w, int(w) if w.isdigit() else default)


@core(r"return the remaining cards? randomly to the bottom of your deck|return any remaining cards? (?:randomly )?to the bottom of your deck|return the rest (?:randomly )?to the bottom of your deck|return the other cards? (?:randomly )?to the bottom of your deck")
...
@core(r"deal (\d+) damage to the first (?:(\d+) )?cards? (?:in|of) (?:your opponent's|the enemy's|the enemy|an enemy|each enemy player's|that player's|the enemy player's) shield area")
...
@core(r"return (.+?) to the (top|bottom) of (?:its owner's|your|their owners'|their owner's|their) decks?")
def _to_deck(m: re.Match[str], g: G) -> list[d.Step]:
    steps, ref = ref_phrase(m.group(1), g)
    return [*steps, d.ToDeck(ref, bottom=m.group(2).lower() == "bottom")]


@core(r"return (.+?) to (?:its owner's|their owners'|their owner's|your) deck and shuffle (?:it|them|that deck|your deck)")
...
@core(r"deploy (\d+|\d+ to \d+)( rested)? " + _TOKDEF + r" Unit tokens?(?: and (\d+)( rested)? " + _TOKDEF + r" Unit tokens?)?")
def _deploy_token(m: re.Match[str], g: G) -> list[d.Step]:
    def one(cnt: str, rested: bool, tdef: str, var: str) -> list[d.Step]:
        ...
        if " to " in cnt:
            ...
            if hi > lo:
                out.append(d.May((d.DeployToken(specs[0].key, hi - lo, rested=rested, var=var + "_x"),)))
            return out
...
def _deploy_from(m: re.Match[str], g: G) -> list[d.Step]:
    ...
    g.it = d.Var(v)
    return [
        d.Choose(v, sel, spec.count, 0 if m.group(1) else None, optional=bool(m.group(1)), targeting=where == "from your trash"),
...
@core(r"place the top (\d+) cards of your deck into your trash|place the top card of your deck into your trash")
def _mill(m: re.Match[str], g: G) -> list[d.Step]:
    g.it = d.Var("milled")
    return [d.Mill(int(m.group(1) or 1))]


@core(r"place the top (\d+) cards? of (?:your opponent's|the enemy's|their) deck into (?:their|your opponent's) trash")
...
def _recover(m: re.Match[str], g: G) -> list[d.Step]:
    ...
    if m.group(3):
        ...
        out += [d.Apply(ref, e, dur or Duration.WHILE_ON_FIELD) for e in continuous_from_predicate(pred, g)]
    return out
...
@core(r"activate (?:the )?【Main】 (?:of|on) the (?:Command )?card paired with this Unit|activate the 【Main】 of its paired Command card")
...
@core(r"(it|they|this Unit|this Base|that Unit|them) (?:won't|will not) be set as active during the start phase of your opponent's next turn")
def _freeze(m: re.Match[str], g: G) -> list[d.Step]:
    _, ref = ref_phrase(m.group(1), g)
    return [d.Apply(ref, d.RuleGrant(d.RuleMod(d.RuleKind.CANT_BE_SET_ACTIVE)), Duration.OPPONENT_NEXT_TURN)]


@core(r"change (?:a|the) battling enemy Unit's attack target to (it|this Unit)|change the attack target of the battling enemy Unit to (it|this Unit)")
...
@core(r"begin a battle between (this Unit|them|it) and (it|them|this Unit)? ?and only perform the damage step|begin a battle between them and only perform the damage step")
...
def _pair_from(m: re.Match[str], g: G) -> list[d.Step]:
    ...
    return [
        d.Choose(v, d.Sel(d.Side.FRIENDLY, loc, spec.sel.filters), 1, optional=bool(m.group(1)), targeting=loc is d.Loc.TRASH),
        d.Pair(d.Var(v), unit),
    ]
...
@core(r"for each (.+?), (it|this Unit) gets (AP|HP)([+-])(\d+) (during this turn|during this battle)")
...
@core(r"reduce (?:its|it's|this Unit's) AP (?:during this (turn|battle) )?by an amount equal to (.+?)(?: during this (turn|battle))?")
...
@core(r"(?:reduce )?the next damage (it|this Unit) receives (?:is reduced )?by (\d+) during this (turn|battle)|reduce the next damage (it|this Unit) receives by (\d+) during this (turn|battle)")
def _next_damage(m: re.Match[str], g: G) -> list[d.Step]:
    ...
    return [d.Apply(ref, d.RuleGrant(rule), Duration.THIS_TURN if which == "turn" else Duration.THIS_BATTLE)]
...
def _reduce_battle_damage(m: re.Match[str], g: G) -> list[d.Step]:
    _, ref = ref_phrase(m.group(1), g)
    rule = d.RuleMod(d.RuleKind.REDUCE_DAMAGE, amount=int(m.group(2)), damage_kind=d.DamageKind.BATTLE)
    return [d.Apply(ref, d.RuleGrant(rule), Duration.THIS_TURN if m.group(3) == "turn" else Duration.THIS_BATTLE)]


@core(r"when (it|this Unit|they) receives? ((?:enemy )?(?:battle )?)damage(?: from an enemy Unit)?, reduce it by (\d+) during this (turn|battle)")
def _reduce_when(m: re.Match[str], g: G) -> list[d.Step]:
    _, ref = ref_phrase(m.group(1), g)
    kind = d.DamageKind.BATTLE if "battle" in m.group(2) or "Unit" in m.group(0) else d.DamageKind.ANY
    side = d.Side.ENEMY if "enemy" in m.group(0) else d.Side.ANY
    rule = d.RuleMod(d.RuleKind.REDUCE_DAMAGE, amount=int(m.group(3)), damage_kind=kind, source_side=side)
    return [d.Apply(ref, d.RuleGrant(rule), Duration.THIS_TURN if m.group(4) == "turn" else Duration.THIS_BATTLE)]


@core(r"(your shield area cards|cards in your shield area) can't receive damage from (.+?) during this (battle|turn)")
...
@core(r"enemy Units can't choose (it|this Unit|them) as their attack target (?:during )?this (turn|battle)")
...
@core(r"all enemy Units must choose (that Unit|it|this Unit) as their attack target(?: if possible)? when attacking during this turn")
def _force_target(m: re.Match[str], g: G) -> list[d.Step]:
    _, ref = ref_phrase(m.group(1), g)
    return [d.Apply(ref, d.RuleGrant(d.RuleMod(d.RuleKind.FORCE_ATTACK_TARGET)), Duration.THIS_TURN)]


@core(r"(?:(.+?) that are Lv\.(\d+) (or lower|or higher)|(.+?)) can't activate <Blocker> during this (battle|turn)")
def _no_block(m: re.Match[str], g: G) -> list[d.Step]:
    phrase = m.group(1) or m.group(4)
    spec = parse_selector("all " + ("enemy " if not phrase.lower().startswith(("enemy", "all", "friendly")) else "") + phrase)
    sel = spec.sel
    if m.group(2):
        sel = d.Sel(sel.side, sel.loc, (*sel.filters, d.StatCmp(d.Stat.LV, cmp_op(m.group(3)), int(m.group(2)))))
...
@core(r"when (it|they|this Unit|that Unit) (destroys? an enemy (?:.*?)Unit with battle damage|destroys? an enemy (?:card|shield area card) with battle damage|deals? battle damage to an enemy Unit.*?)(?: during this turn)?, (.+)")
def _delayed_when(m: re.Match[str], g: G) -> list[d.Step]:
    who = m.group(1).lower()
    subject_text = "this Unit " + re.sub(r"^destroy\b", "destroys", re.sub(r"^deal\b", "deals", m.group(2)))
...
@core(r"(it|they|them|this Unit|this Base|this|that Unit|the enemy Unit|the attacking enemy Unit|all .+?|enemy Units.*?|friendly Units.*?|your Units.*?|all your .+?|(?:\d+|a) .+?) (gets? .+|gains? .+|can't .+|may choose .+|can choose .+|may attack .+|can attack .+)")
...
def _compile_option_block(lead: str, options: list[str], g: G, steps: list[d.Step]) -> None:
    ...
    m = re.match(r"^You may exile the specified number of (.+?) in your trash from the game\. If you do, activate the following effect$", lead_s)
...
def compile_sentence(sentence: str, g: G, steps: list[d.Step]) -> None:
    ...
    if m and not re.match(r"^if (it is|that card is) .+?, (?:you may )?reveal it", s, re.I):
        ...
        if cond is not None:
            ...
            steps.append(d.If(cond, tuple(inner)))
            return
    ...
    if m and not re.match(r"^(?:reveal|choose)", s, re.I):
        try:
            cond = parse_condition(m.group(2), g)
        except CompileError:
            cond = None
        if cond is not None:
            inner = []
...
def parse_costs(text: str, g: G) -> tuple[d.Cost, ...]:
    ...
    for part in re.split(r",\s*|\s*･\s*", text.strip()):
        ...
        if m:
            spec = parse_selector(m.group(1) + " " + m.group(2))
            out.append(d.ExileCards(d.Sel(d.Side.FRIENDLY, d.Loc.TRASH, spec.sel.filters), int(m.group(1))))
            continue
...
def compile_constant_sentence(s: str, g: G, cond: d.Cond, gate: d.Gate, gf: tuple[d.Filter, ...]) -> list[d.Ability]:
    s = _strip_period(s)
    ...
    if m:
        return [d.Constant((d.CostMod(cost=int(m.group(2)), level=int(m.group(1))),), d.This(), cond, where=d.Where.HAND)]
    m = re.match(r"^This card's name is also treated as \[(.+)\]$", s)
    if m:
        raise CompileError("name alias requires a binding")
    m = re.match(r"^(" + _CONST_SUBJECT + r") (gets? .+|gains? .+|can't .+|may choose .+|can choose .+|can attack .+|may attack .+)$", s)
    if m:
        subject = m.group(1)
    ...
    m = re.match(r"^Enemy Units choose this (?:rested )?Unit as their attack target if possible(?: when attacking)?$", s)
    if m:
        return [d.Constant((d.RuleGrant(d.RuleMod(d.RuleKind.FORCE_ATTACK_TARGET)),), d.This(), cond, gate=gate, gate_filters=gf)]
    m = re.match(r"^Enemy Units can't choose this Unit as their attack target$", s)
    if m:
        return [d.Constant((d.RuleGrant(d.RuleMod(d.RuleKind.CANT_BE_ATTACKED)),), d.This(), cond, gate=gate, gate_filters=gf)]
    m = re.match(r"^On the turn this Unit is deployed, it may choose a rested enemy Unit as its attack target and attack it$", s)
    if m:
        rule = d.RuleMod(d.RuleKind.ATTACK_ON_DEPLOY_TURN, name="units_only", source_filters=(d.IsRested(True),))
        return [d.Constant((d.RuleGrant(rule),), d.This(), cond, gate=gate, gate_filters=gf)]
    raise CompileError(f"unknown constant {s!r}")


def compile_untagged(body: str, g: G, gate: d.Gate, gf: tuple[d.Filter, ...], once: bool) -> list[d.Ability]:
    ...
    if m:
        red = _damage_reduction(m.group(1), m.group(2), sentences[1:], g, once, gate, gf, scope_cond)
        if red is not None:
            return red
        ...
        return [d.Triggered(trig, steps, once_per_turn=once, gate=gate, gate_filters=gf, where=where)]
    out: list[d.Ability] = []
...
def _damage_reduction(
    ...
    kind = d.DamageKind.BATTLE if "battle" in kind_txt else (d.DamageKind.EFFECT if "effect" in kind_txt else d.DamageKind.ANY)
    side = d.Side.ENEMY if "enemy" in kind_txt else d.Side.ANY
    rule = d.RuleMod(d.RuleKind.REDUCE_DAMAGE, amount=int(m.group(1)), damage_kind=kind, source_side=side, once_per_turn=once)
    subj = t.group(1).lower()
    scope: d.Ref = d.This()
    if subj not in ("this unit", "this base"):
        spec = parse_selector("all " + re.sub(r"^(one of your|a friendly) ", "friendly ", t.group(1), flags=re.I))
        scope = d.All(spec.sel)
...
def _compile_timed(
    ...
    if trig is not None:
        g.event = trig.event
        return [d.Triggered(trig, compile_steps(body, g), once_per_turn=once, gate=gate, gate_filters=gf)]
    if t in ("Activate･Main", "Activate･Action"):
        ...
        return [d.Activated(timing, costs, compile_steps(body, g), once_per_turn=once, gate=gate, gate_filters=gf)]
    if t in ("Main", "Action"):
        return [d.Command(d.Timing.MAIN if t == "Main" else d.Timing.ACTION, compile_steps(body, g))]
